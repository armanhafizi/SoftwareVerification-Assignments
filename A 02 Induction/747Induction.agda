...
-- helper 1: multiplication right zero
*-zeroʳ : ∀ (m : ℕ) → m * zero ≡ zero
*-zeroʳ zero = refl
*-zeroʳ (suc m) rewrite *-zeroʳ m = refl

-- helper 2: muliplication right successor
*-suc : ∀ (m n : ℕ) → m * suc n ≡ m + m * n
*-suc zero n = refl
*-suc (suc m) n rewrite *-suc m n | +-comm n (m + m * n) | +-swap m (m * n) n | +-comm m (n + m * n) | +-swap n (m * n) m | +-comm m n = refl -- not sure
...
to/from-corr .(tob n) n refl rewrite from∘tob n = refl -- not sure if this counts
...
to∘+ zero n = refl
to∘+ (suc m) n rewrite bin-+-linc (tob m) (tob n) | to∘+ m n = refl
