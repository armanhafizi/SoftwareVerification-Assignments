...
dblb∘inc : ∀ (m : Bin-ℕ) → dblb (inc m) ≡ inc (inc (dblb m)) -- helper 1 from 747Induction
dblb∘inc bits = refl
dblb∘inc (m x0) = refl
dblb∘inc (m x1) = refl

to∘dbl : ∀ (m : ℕ) → tob (dbl m) ≡ dblb (tob m) -- helper 2 from 747Induction
to∘dbl zero = refl
to∘dbl (suc m) rewrite to∘dbl m | dblb∘inc (tob m) = refl

-- 747/PLFA exercise: OneToFrom (3 points)

one-to∘from : ∀ {n : Bin-ℕ} → One n → tob (fromb n) ≡ n
one-to∘from [bitsx1] = refl
one-to∘from {n x0} (on [x0]) rewrite to∘dbl (fromb n) | one-to∘from on | dblb-x0 on = refl
one-to∘from {n x1} (on [x1]) rewrite to∘dbl (fromb n) | one-to∘from on | dblb-x0 on = refl
...
from∘inc : ∀ (m : Bin-ℕ) → fromb (inc m) ≡ suc (fromb m)  -- helpers from Induction.agda
from∘inc bits = refl
from∘inc (m x0) = refl
...
from∘to iso-ℕ-CanR zero = refl
from∘to iso-ℕ-CanR (suc x) rewrite from∘inc (tob x) | from∘to iso-ℕ-CanR x = refl

to∘from iso-ℕ-CanR (mk-CanR n y) = rewrap (to-can (fromb n)) y (can-to∘from y)
