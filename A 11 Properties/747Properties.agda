...
_≃_.to (progress-iso {M}) (step {N} x) = inj₂ ⟨ N , x ⟩
_≃_.to (progress-iso {M}) (done x) = inj₁ x
_≃_.from (progress-iso {M}) (inj₁ x) = done x
_≃_.from (progress-iso {M}) (inj₂ ⟨ fst , snd ⟩) = step snd
_≃_.from∘to (progress-iso {M}) (step x) = refl
_≃_.from∘to (progress-iso {M}) (done x) = refl
_≃_.to∘from (progress-iso {M}) (inj₁ x) = refl
_≃_.to∘from (progress-iso {M}) (inj₂ y) = refl

progress′ : ∀ M {A} → ∅ ⊢ M ⦂ A → Value M ⊎ ∃[ N ](M —→ N)
progress′ (ƛ x ⇒ N) (⊢ƛ m:a) = inj₁ V-ƛ
progress′ (V · M) (m:a · m:a₁) with progress′ V m:a
... | inj₂ ⟨ L′ , snd ⟩ = inj₂ ⟨ L′ · M , ξ-·₁ snd ⟩
... | inj₁ ValV with progress′ M m:a₁
... | inj₂ ⟨ M′ , snd ⟩ = inj₂ ⟨ V · M′ ,  ξ-·₂ ValV snd ⟩
... | inj₁ ValM with canonical m:a ValV
... | C-ƛ {x} {A} {N} {B} r = inj₂ ⟨ N [ x := M ] , β-ƛ ValM ⟩
progress′ .`zero {.`ℕ} ⊢zero = inj₁ V-zero
progress′ (`suc M) {.`ℕ} (⊢suc m:a) with progress′ M m:a
... | inj₁ ValM = inj₁ (V-suc ValM)
... | inj₂ ⟨ fst , snd ⟩ = inj₂ ⟨ `suc fst , ξ-suc snd ⟩
progress′ (case L [zero⇒ M |suc x ⇒ N ]) {A} (⊢case m:a m:a₁ m:a₂)  with progress′ L m:a
... | inj₂ ⟨ L′ , snd ⟩ = inj₂ ⟨ case L′ [zero⇒ M |suc x ⇒ N ] , ξ-case snd ⟩
... | inj₁ ValL with canonical m:a ValL
... | C-zero = inj₂ ⟨ M , β-zero ⟩
... | C-suc {V} CanV = inj₂ ⟨ N [ x := V ] , β-suc (value CanV) ⟩
progress′ (μ x ⇒ M) {A} (⊢μ m:a) = inj₂ ⟨ M [ x := μ x ⇒ M ] , β-μ ⟩
...
... | step x = false because (ofⁿ (—→¬V x))
... | done x = true because ofʸ x
...
unstuck : ∀ {M A}
  → ∅ ⊢ M ⦂ A
    -----------
  → ¬ (Stuck M)
unstuck {M} {A} m:a ⟨ fst , snd ⟩ with progress m:a
... | step x = fst x
... | done x = snd x
-- —→¬V
preserves : ∀ {M N A}
...
preserves {M} {.M} {A} m:a (.M ∎) = m:a
preserves {M} {N} {A} m:a (.M —→⟨ x ⟩ msn) = preserves (preserve m:a x) msn
...
wttdgs : ∀ {M N A}
  ...
  → M —↠ N
    -----------
  → ¬ (Stuck N)
wttdgs m:a msn x = unstuck (preserves m:a msn) x
