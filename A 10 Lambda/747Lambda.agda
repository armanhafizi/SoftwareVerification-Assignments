...
mul = μ "*" ⇒ ƛ "m" ⇒ ƛ "n" ⇒
        case ` "m"
          ...
          |suc "m" ⇒
            case ` "n"
              [zero⇒ `zero
              |suc "n" ⇒ `suc (plus · (plus · ` "m" · ` "n") · (` "*" · ` "m" · ` "n")) ] ] -- steps: (m+1) * (n+1) = m*n + m + n + 1 = suc ((m+n) + (m*n))
...
mulᶜ = ƛ "m" ⇒ ƛ "n" ⇒ ƛ "s" ⇒ ƛ "z" ⇒
             ` "m" · (` "n" · ` "s" ) · ` "z" -- first apply s for n times to get +n func. Then apply +n func for m times starting from z to get m*n
...
trans : ∀ {L M N} -- helper
    ...
    → L —↠ N
trans (_ ∎) MN = MN
trans {L} (_ —→⟨ LX ⟩ XM) MN = L —→⟨ LX ⟩ trans XM MN

ms1≤ms2 : ∀ {M N} → (M —↠ N) ≲ (M —↠′ N)
to ms1≤ms2 (_ ∎) = refl′
to ms1≤ms2 (_ —→⟨ x ⟩ x₁) = trans′ (step′ x) (to ms1≤ms2 x₁)
from (ms1≤ms2 {M} {N}) (step′ x) = M —→⟨ x ⟩ N ∎
from (ms1≤ms2 {M} {N}) refl′ = M ∎
from (ms1≤ms2 {M} {N}) (trans′ x x₁) = trans (from ms1≤ms2 x) (from ms1≤ms2 x₁)
from∘to ms1≤ms2 (_ ∎) = refl
from∘to (ms1≤ms2 {M} {N}) (_ —→⟨ x ⟩ x₁) rewrite from∘to ms1≤ms2 x₁ = refl
...
⊢mul : ∀ {Γ} → Γ ⊢ mul ⦂ `ℕ ⇒ `ℕ ⇒ `ℕ
⊢mul {Γ} = ⊢μ (⊢ƛ (⊢ƛ (⊢case (⊢` (S′ Z)) ⊢zero (⊢case (⊢` (S′ Z)) ⊢zero (⊢suc (⊢μ (⊢ƛ (⊢ƛ (⊢case (⊢` (S′ Z)) (⊢` Z) (⊢suc (((⊢` (S′ (S′ (S′ Z)))) · ⊢` Z) · ⊢` (S′ Z)))))) · (((⊢μ (⊢ƛ (⊢ƛ (⊢case (⊢` (S′ Z)) (⊢` Z) (⊢suc (((⊢` (S′ (S′ (S′ Z)))) · ⊢` Z) · ⊢` (S′ Z))))))) · (⊢` (S′ Z))) · (⊢` Z)) · (((⊢` (S′ (S′ (S′ (S′ Z))))) · (⊢` (S′ Z))) · ⊢` Z)))))))
