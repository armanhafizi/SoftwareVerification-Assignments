...
++-identityʳ′ : ∀ {A : Set} (xs : List A) → xs ≡ xs ++ [] -- helper
++-identityʳ′ [] = refl
++-identityʳ′ (x ∷ xs) rewrite ++-identityʳ xs = refl

reverse-++-commute : ∀ {A : Set} (xs ys : List A)
 → reverse (xs ++ ys) ≡ reverse ys ++ reverse xs
reverse-++-commute xs [] rewrite ++-identityʳ xs = refl
reverse-++-commute [] (y ∷ ys) = ++-identityʳ′ ((reverse ys) ++ [ y ])
reverse-++-commute (x ∷ xs) (y ∷ ys) rewrite reverse-++-commute xs (y ∷ ys) | ++-assoc ((reverse ys) ++ [ y ]) (reverse xs) [ x ] = refl
...
map-++-dist f xs [] rewrite ++-identityʳ xs | ++-identityʳ (map f xs) = refl
map-++-dist f [] (y ∷ ys) = refl
map-++-dist f (x ∷ xs) (y ∷ ys) rewrite map-++-dist f xs (y ∷ ys) = refl
...
_ : foldr  _∸_ 1 [ 100 , 4 , 3 , 2 ] ≡ 98 -- just an example to check correctness of foldr
_ = refl

_ : foldl  _∸_ 100 [ 4 , 3 , 2 , 1 ] ≡ 90 -- just an example to check correctness of foldl
_ = refl
...
foldl-arb : ∀ {A : Set} → {{m : IsMonoid A}} → ∀ (e : A) → -- helper as suggested above
  ∀ (xs : List A) → foldl _⊗_ e xs ≡ (e ⊗ foldr _⊗_ id xs)
foldl-arb e [] rewrite identityʳ e = refl
foldl-arb e (x ∷ xs) rewrite foldl-arb (e ⊗ x) xs = assoc e x (foldr _⊗_ id xs)

foldl-r-mon : ∀ {A : Set} → {{m : IsMonoid A}} →
  ∀ (xs : List A) → foldl _⊗_ id xs ≡ foldr _⊗_ id xs
foldl-r-mon [] = refl
foldl-r-mon (x ∷ xs) rewrite identityˡ x = foldl-arb x xs
